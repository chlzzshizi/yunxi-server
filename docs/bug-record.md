...
## Bug 36：`store_id=NULL` 的店长建不了一单，报的还是句假话（**本轮发现，2026-09-19 已修**）
...
### 补记（2026-09-19）：拍板"说实话，仍然拒绝"，已修

上面"为什么没修"列的三个方向里，**取 ②（话说对）与 ③（现状照旧）的并集，不动 ①** ——
即**仍然拒绝他建门店单**，只把话说成真的。用户原话是「说实话，仍然拒绝」。

**为什么不选 ①（建号时对 `role=1` 强制 `storeId` 非空）**：那等于用一个校验去取消一个
**已经拍过板的口径**（门店不做数据隔离 ⇒ 归属门店对店长不是权限边界，只是"这单算哪家店"），
代价还落在**建号**这条无辜的路上 —— 而真正缺的东西只是**一句准确的话**。
`CreateStaffRequest.storeId` 那行注释至今写着「**店长允许为空**」，本轮没动它。

**"拒绝"保留的理由**：门店单要有**物理的店**（取件地址落在哪家），这不是权限问题，
是"这单不知道算哪家"的问题。所以拒绝本身是对的，错的只是**给的理由**。

**改了什么**（`OrderController.requireStaffStore`，原 `:217-223` → 现 `:229-235`）：

- `throw new BusinessException(401, "登录信息已升级，请重新登录")`
  → `throw new BusinessException(403, "账号还没有归属门店，无法开单，请联系管理员分配门店")`
- **为什么是 403 不是 401**：401 在本项目里到处都意味着"重新登录"，客户端会照着去做一件
  **没有用**的事（重登一万次也不会带上门店）。403 说的是"你的身份我认了，但这个动作不行"。
  与 Bug 42 那条"401 和 403 是两个意思，不能混用"是同一条规矩。
- 那段 javadoc 里**假的**全称断言（"只剩**旧 token**"）也一起改了：现在写的是
  "**只可能是无店店长**"，并给出**旧 token 已绝迹**的证据 —— 票有效期 24 小时
  （`application.yml` 的 `jwt.expiration=86400000`），storeId 是 2026-09-11 随角色签进 claim 的，
  那之前的票 **09-12 就全过期了**。所以这一支今天只可能是"按设计就没有门店"的账号。

**还留着什么（说清楚，别以为修完他就能干活了）**：这个账号**开不了任何单** ——
门店单被这道闸拒（要有物理的店），网单要顾客身份（走 `requireCustomer`，他进不去）。
他能做的仍只有"登录 + 读"。**要干活得让管理员给他分一个门店** —— 而这正是新文案让他去做的事。

**证据（实测，环境：本机 / Windows / 热库）**：

- 单测 **401 → 405**：新增 `OrderControllerTest`（4 条）—— 无店 → 403 + 逐字文案 + `never()` 确认
  一步没进应用层；**否定形状的一条**（文案里不许再出现「重新登录」）；
  两条对照（有店店长照常建单、token 里的门店不被请求体顶掉）。
- 九个脚本 **393 → 397**（`verify-orders.sh` **70 → 74**，A 段新增 4 条：A6 → 403 / A6b 文案 /
  A6c 不许说「重新登录」/ A6d 对照读价目表 → 200），**九个退出码全 0**。
  A6 的脚手架是**用管理员的接口建**这号（不是 SQL）—— 那才顺带证明了
  "无店店长是一条走得通的正路"，而不是把自己写下的前提当前提。
- 反向探针（改**副本**，三条都真红了、都退 1）：把期望文案换回旧话 → A6b FAIL；
  把准备守卫的条件取反 → `[准备失败]`；把 A6c 的否定模式换成响应里真有的片段 → A6c FAIL。

**这条记的是"改话"这种修法的形状**：一处**没人会去测**的地方（注释里的全称断言 +
给用户看的文案）也能是 bug 的主体，而它**只有那一条路能走到** —— 脚本要走到得先建号再登录，
单测一行 mock 就到现场。所以这次先写单测，再补真机探针。
...
### 拍板与第三道闸（2026-09-19：**数量必须为正整数**）

上面那段 ⚠️ 边界挂了一天，2026-09-19 用户拍板：**拦**，判据 `quantity <= 0`（0 和负数都拒）。

**先更正上面 ⚠️ 里的一句**。原文写"负总额会在 `pay` 那里被'不能为负数'拦下，走不远" ——
**这句不成立**：`pay` 拦的是**总额为负**，而一张"一件 100 元 + 一条 -1 件×100"的单子总额恰好是 0，
**不是负数**，一路畅通（负数真正的坏处不是把总额弄负，是能把别的明细**对冲**掉：
少付钱还判"付清了"）。算错的地方在于我当时只看了"总额的符号"，没看"总额是怎么凑出来的"。

**闸落在两处（接口层那道原地不动，共三道）**：

| 层 | 位置 | 判据 |
|---|---|---|
| 接口层 | `OrderController.java:59`（**原有**，本次没动） | `quantity == null \|\| <= 0` → 400「第 N 条明细数量必须为正整数」 |
| 应用层 | `OrderAppService.toItems`（本次加） | 同上，文案**逐字相同** |
| 域层 | `Order` 构造器（本次加，紧挨着空明细那道闸） | 同上，带序号 |

**为什么落两处而不是一处**：应用层那段注释自己立过规矩 ——"应用服务是**用例的入口**，
将来还会有别的入口（定时任务、后台脚本、别人写的第二个前端）调它，校验不能押在某一个入口的自觉上" ——
它当时只查了 `quantity == null`（缺字段），**没查"值不合法"**；域层同理，只查了空集不查元素。
两处都是"自己说了要做、但只做了一半"。

**为什么闸不在 `OrderItem` 构造器**：那个带单价的构造器是**从库里恢复数据**用的，
规则塞进去，历史脏数据一读就炸 —— 而这不是它该管的事，建单才是不变量成立的地方。

**验证（实测）**

- 单测 **401**（common 12 / domain 98 / application 238 / interfaces 53），`./mvnw -o -B test` 全绿；
  新增 6 条：`OrderTest.NonPositiveQuantityRejected` 4 条（0 件 / 负数 / **坏的在第 2 条就报第 2 条** /
  **对照：1 件和 2 件照常建**）、`OrderAppServiceTest.rejectNonPositiveQuantity` +
  `quantityOneStillWorks` 2 条（含"在算价之前拦下"，`verify(priceRepository, never())`）
- 九个脚本**全量重跑 393 / 393、九个退出码全 0**（orders **69 → 70**，新增 **C2b**：
  数量 -1 → 400。C2 只管 0，**判据写 `<= 0` 还是 `== 0` 只有 C2b 分得开**）
- **反向探针**：/tmp 副本上把 C2b 的请求改成 `quantity:1`（期望的 400 就不来了）→
  `[FAIL] C2b` + **退出码 1**（69 通过 / 1 失败），仓库里的脚本没动
- `OrderItemTest` 那条「数量 0 → 0.00」**保留**（它钉的是明细的纯算术），
  但注释更新了：它不再是"规则只在下单入口"的证据，而是"为什么必须要有那道闸"的理由

**分工写清楚**：C2b 证的是**最外层那道闸**（400 文案）。过了 controller 之后是应用层还是域层拦的，
脚本看不出来 —— 那两道只有单测能证。也就是说这次的本机全绿**并不证明**新增的两道闸在真机上生效
（HTTP 层它们的判据和 controller 那条完全一样，观察不到差别）；它们证的是
**"换定时任务、脚本、第二个前端这些入口进来同样绕不掉"**。
...
## Bug 48：默认账号 `admin` 登进去处处 403 —— **管理员那半边前端根本没写**（2026-09-19 发现，当天补页面）

### 现象

照员工登录页的引导登录（它**默认就填着 `admin`**，文案写着"演示账号 admin / admin123"），
进去是一屏接一屏的 403：订单列表 403、建单 403、定价 403、发券台 403。

`admin` 是这个项目 README 里公开的两个演示账号之一，所以"照文档登录"恰好是踩进这个坑最短的路径。

### 根因：不是后端坏了，是前端缺了一半

后端的权限口径**一直是对的**（2026-09-11 拍板）：`JwtInterceptor.checkRoleGate` 第二方向
把 `/api/orders/**`、`PUT /api/prices`、`POST /api/coupons`、`/api/customers/**`
对 ADMIN 一律 403 —— 管理员只管人与店，不碰订单。

而 ADMIN **唯一能干的那套接口**，前端一个页面都没有：

| 接口 | 前端现状（2026-09-19 之前） |
|---|---|
| `GET/POST /api/staff`、`GET/PUT /api/staff/{id}`、`PUT …/password`、`PUT …/status` | 无页面，连 `api/staff.js` 都没有 |
| `POST /api/stores`、`PUT /api/stores/{id}`、`PUT …/status`、`GET /api/stores/all` | 无页面（`api/store.js` 只有"读营业中门店"那一个函数） |

于是"管理员"这个角色在前端只存在于一个地方：一个**能被登进去、之后无处可去**的账号。

**它一路躲过了所有自动检查**：`npm run build` 不报（页面压根不存在，就没有东西会编译错）、
单测不报（测的是后端）、九个真机脚本不报（打的是接口，接口全是好的）。
和 Bug 28 是同一个形状的另一半 —— 那条说的是"构建绿不证明按钮能用"，
这条说的是**"构建绿也从来不证明这一半功能存在"**。

### 修法（2026-09-19）

1. **补上管理员那半边**：`/staff/admin/staff`（员工管理：名册 / 新建 / 编辑 / 重置密码 / 启停用）
   与 `/staff/admin/stores`（门店管理：建店 / 改店 / 营业停业），外加 `api/staff.js` 六个接口、
   `api/store.js` 四个写接口 + `listAllStores`
2. **登录后按角色分流**：新增 `router.homeFor(persona, session)` 读 JWT 里的 `role`
   —— 管理员 → `/staff/admin/staff`，店长 → `/staff/orders`。
   守卫两处 + 登录页一处**都问它**，全局没有第二份"落地页"。
   顺带把守卫 `meta.guest` 那支也改成走 `homeFor`：不改的话管理员点"员工登录"
   会被弹回满屏 403 的订单页 —— 正是本条要消灭的那个体验
3. **默认账号改成 `manager`**：登录页的默认值该指向多数人要走的那条路（店长），
   文案说清两种角色各能干什么，并写明"订单/定价/发券对管理员是 403，是口径不是坏了"
4. **`meta.adminOnly` 只拦一个方向**：店长进 `/staff/admin/*` → 弹回自己的首页
   （进去只有一屏 403）；**管理员进订单页不拦** —— 那是后端的如实口径，前端不替他决定。
   `router/index.js` 里"不按 role 藏按钮"那段旧结论**一字未删**，只补了这两处例外

### 验证（**本轮没有真机验证**）

- ✅ `npm run build` 通过（4 个新页各自出 chunk：`StaffManageView` / `StoreManageView` /
  `CustomerCouponView` / `CustomerProfileView`）
- ✅ 八项校验常量与后端逐一对上（用户名 30 / 姓名 20 / 员工手机号 20 / 店名 50 /
  地址 200 / 门店电话 20 / 顾客姓名 20 / 密码 72 字节），文案逐字比对过；
  连"员工那边叫**手机号**、门店那边叫**电话**"这个差别也照抄了
- ❌ **页面能不能点、角色分流对不对、表单过不过后端闸门 —— 一次都没跑过。**
  本轮约定：**用户自己点，AI 不起任何服务**（后端 8081 / dev server 都不起）。
  所以这条按 Bug 29 的规矩办：**不写"已修好"，只写"代码已就位、验证待做"**；
  "哪些地方没验证过"的清单随交付一并给出

> ### 验证补记（2026-09-19 当天稍晚，**真机点过了**）
>
> 上面那句"一次都没跑过"在当天就作废了。用**无头 Chrome**（系统自带的，
> 一个浏览器都没下载；探针装在仓库外，`yunxi-web/package.json` 没动）真点了一遍：
> **44 条断言全过，页面零 JS 报错 / 零请求失败**。
>
> | 原来"没验证过"的 | 实测 |
> |---|---|
> | 角色分流落地页 | 店长 → `/staff/orders`；管理员 → `/staff/admin/staff`（名册 44 行渲染） |
> | `meta.adminOnly`（单向） | 店长进两个 `/staff/admin/*` 都被弹回订单页；**管理员进订单页不拦**，如实显示「管理员不参与订单操作，请使用店长账号」 |
> | 守卫 `meta.guest` 那支 | 已登录的管理员点"员工登录" → 弹到**管理员首页**（不是满屏 403 的订单页）—— 本轮改的那处确实生效 |
> | 401 链路 | 后台把票作废 → 本地会话被清 → 自动回登录页 |
> | 表单过不过后端闸门 | 三块面板真点：新建（**姓名 21 字被前端当场拦下，且一次 POST 都没发**）/ 编辑（用户名只读）/ 重置密码（空密码被拦；成功提示含"他手上的登录已作废"）/ 停用；门店：建店 / 改电话 / 停业 |
> | 顾客两个新页 | 注册只有 2 个字段 → 注册即登录；我的券渲染；个人中心手机号只读、补姓名保存后**刷新还在**（真落库） |
>
> **顺带撞到一件不在本条范围内的事**：Bootstrap CDN（`cdn.staticfile.org`）在约 60 次
> 页面加载里失败了 1 次（`ERR_CONNECTION_CLOSED`），**那一次整页没有样式**；专门打 10 次
> 0 失败 —— 偶发，但真会发生。`index.html` 已给 CSS 与 JS 各挂一条备用链（bootcdn），
> 并**掐断 staticfile 验证过**：样式照样生效。注意两种写法不一样 —— `link` 改 `href` 会重新
> 加载，`script` 改 `src` **不会**（已启动的 script 必须新建元素），这里踩过一次。
>
> 过程中红了 6 条断言，**全是探针脚本自己的错**（选择器数错位、断言成 `/customer/home`
> 而实际口径是 `/customer/orders`、手机号重复注册、跨人格时没清会话）——没有一条是应用的。

### 教训

**一个角色"能登进去但无处可去"，用户看到的就是系统坏了 —— 而这件事没有任何自动检查会管。**
`build` / 单测 / 九脚本各自的盲区在这里正好叠成一个完整的洞：
它们都只检查"已经存在的东西对不对"，**没有一样在问"该有的东西在不在"**。
缺页面这件事，只有"点一遍"能发现。

---

## Bug 49：编辑面板**拿名册行回显**，保存却交整份快照 —— 一根写好了没人接的线（2026-09-19 发现，当晚接上）

> **性质：潜伏缺陷（latent）。** 没有"今天坏了"，是**并发下会悄悄改错别人的数据**，
> 而唯一能防住它的那个接口，前端写了包装函数却**从来没人 import**。

### 现象（潜伏，日常单人用看不出来）

`StaffManageView.vue` 的编辑面板从**名册行**填表（`openEdit(row)`），
而提交走的是 `PUT /api/staff/{id}` —— **整份快照**（`name` / `role` / `storeId` / `phone` 全交）。
名册是本页打开时拉的，之后就再也不刷新（除非自己保存一次）。

于是：管理员 A 的页面开在那儿 5 分钟 → 期间管理员 B 改了员工 X 的手机号 →
A 点开 X 的编辑面板（看到的是**旧手机号**）→ 只改了姓名就保存 →
**B 刚改的手机号被 A 悄悄改回去了**。不报错、不留痕、两个人谁都不知道。

单人单设备用不出来：这一页的绝大多数操作（新建/改名/停用）自己一保存就会 `loadList()` 刷新名册。
**只有"页面开着不动 + 别人动了手"这条路径才露出来。**

### 根因：接口、包装函数、注释三样都在，**中间那根线没接**

- 后端 `GET /api/staff/{id}` 一直是好的（`StaffAdminAppService.getOne`，只认 ADMIN）
- `api/staff.js:20-23` 的注释**已经写明它是干什么的**：`/** 单个员工 —— 编辑面板回显用 */`
- 而它全仓库零 import —— 编辑面板直接拿行数据填了，没人用它

这条**在 Bug 48 的补页面工作之后依然存在**：Bug 48 那张表把 `GET /api/staff/{id}`
记成"无页面，连 `api/staff.js` 都没有"；页面补上了，包装函数也写出来了，
**但这一根还是断的** —— "有页面"不等于"页面里每个函数都接了线"。

**怎么发现的**：不是点出来的，是**数出来的** —— 把 `src/` 下每个 `export` 的引用全量扫一遍，
逐个函数名 `grep -rw` 数出现次数。`getStaff` 只有定义那一行。**`npm run build` 对此完全无感**：
导出没人用，不报错、不警告、不进产物差异。

> **同一轮扫描里的两处更正（免得后面再有人照着念）**：
> - `charCount` / `decodeToken` **不是死码** —— 它们被**同一个文件里**的函数用着
>   （`tooLong()` 用前者、`loadSession()`/`saveSession()` 用后者），只是**多写了 `export`**。
>   真正"全仓库零引用"的只有 `getStaff` 一个。我早先口头把这三个一起说成"死导出"，**不准确**，此处更正。
> - 扫描工具本身踩过一次坑：`grep -oP` 在这台机器的 Git Bash 上不支持，`2>/dev/null` 把报错吞了 →
>   函数名列表为空 → 循环没跑 → **输出"没有死函数"**。假绿。改用 `sed` 才查出真结果。
>   （与 Bug 24/25/38/40 同族：**"没输出"和"没东西"长得一样**。）

### 修法（用户指令："先解决 getstaff 的问题"→ 选择**接上**，不是删掉）

删掉是另一个选项（6 行，零风险）。**没选它**，因为那不只是删个函数，是**把一条已知的错数据路径留在那里**：
接口和注释都写着"编辑面板回显用"，说明它本来就是为这个场景写的；删了，将来要修同一个问题还得重写一遍。

`openEdit` 拆成两半：

```js
function openEdit(row) {
  fillEditForm(row)   // ① 先用名册行把面板开出来 —— 点了就有，不等后端
  formError.value = ''; formOk.value = ''; panel.value = 'edit'
...
三个细节：
1. **两段式**：先填行再拉详情 —— 面板"点了就有"，不为了一个几十毫秒的回环把点击变得发涩。
2. **竞态守卫**（`panel === 'edit' && target.id === row.id`）：A 的响应慢、用户中途点了 B，
   迟到的 A 不能把 B 的面板覆盖掉。**这是这段代码唯一真正容易写错的地方**，有专门断言。
3. **失败就地报错**：详情取不到时用行数据照样能编辑，红条提示，不白屏。

### 验证（**21 条断言，真机无头 Chrome 跑过**；环境：本机 Windows / 热库 / 后端 8081 + Vite 5173）

三个探针（都装在仓库外 `%TEMP%\webtest\`，`yunxi-web/package.json` 没动）：

| 探针 | 条数 | 覆盖 |
|---|---|---|
| `getstaff.js` | 8 | ① 不拦包：点"编辑"**真发出** `GET /api/staff/137 → 200`（这个端点此前从没有前端调过），面板值与详情逐字一致；② **拦包换假名字**：面板必须显示假名字**而名册行仍是旧值** —— 这一条是核心，它把"表单来自响应"和"碰巧跟行一样"分开了；③ A 的响应拖慢 1.5 秒、中途切去 B，面板最后属于 B；④ 详情让 `route.abort()`，红条 `网络异常：请确认后端 8081 已启动`，行数据还在、面板照常能用 |
| `edit-regress.js` | 9 | 改 `fillEditForm` 之后**保存这条路没断**：R5 改的确实是 id=137 那条、R6 一个无关员工（admin）**字节级没动**、R8 其它字段没被抹掉；R2 详情里的 role 名字（`'ADMIN'`）被映射回数字 0；R3 role=ADMIN ⇒ 门店下拉禁用。**写库只动了自己探针号的 `name`，改完 R9 还原成原名**（`探针遗留-可删`）|
| `edit-store.js` | 4 | **危险面**：详情响应若缺 `storeId`，"打开面板 → 直接保存"会把店长的门店清成 null。实测详情字段集与名册行**完全相同**（`id,username,name,role,storeId,storeName,phone,status`），真店长 `manager`(id=2, storeId=1) 打开面板门店下拉照旧选中 |

**没验证的**：并发那条路径本身（"A 开页面时 B 改数据"）**没有真造两个管理员去撞** ——
②③④ 是拦包/拖慢模拟出来的，证的是"响应会覆盖表单""迟到的不覆盖""失败能降级"这三条机制，
**不是**"真有两个人在并发时一定不出错"。机制在，端到端的那一幕没导演过。

### 教训

**Bug 48 说"没有一样检查在问该有的东西在不在"；这一条是它的下一层 —— "页面有了"也不等于"页面里的线都接了"。**
`build` 会对**语法**说话，对**"这个函数从来没人用"**一言不发；
单测和九脚本打的是后端接口，更看不见。能发现它的只有**去数引用**，或者去点。
本轮两条路都走了，结论一致 —— 但两次都是"人/ AI 主动去查"，**没有一条是自动门禁替我发现的**。
...
| 36 | Interfaces | 组合出的**假成因**（本轮发现，**2026-09-19 已修**） | 允许建 `store_id=NULL` 的店长（合法状态）× `requireStaffStore` 的 401「登录信息已升级」→ 无店店长建单收到一句假话。**它上面那段注释写的"只剩旧 token 会落在这里"因此变成假的**。2026-09-19 拍板「说实话，仍然拒绝」：**照拒**（门店单要有物理的店），401 → **403**「账号还没有归属门店，无法开单，请联系管理员分配门店」，注释同步改成真话（旧 token 那条路已绝迹：24h 有效期） |
...
| 43 | Domain | 状态机 + 金额缺口（**发现当晚已修**） | 空明细 → 总额 0 → `pay(0)` 同时满足"等于全额"和"等于 0" → `requirePaidOff` 问的是"够不够"，`0 < 0` 为假 → **白洗到终态 7，一分钱没收**。四个环节分开看都"没错"。原先唯一的闸在 `OrderController`（最外层，换入口就绕过）→ 已把闸补进 `Order` 构造器。**边界：`quantity=0` 仍是同一条路，本轮没动**（口径要人定）。**2026-09-19 拍板：拦**，判据 `<= 0`（0 与负数都拒）—— 应用层 `toItems` 与域层 `Order` 构造器各补一道（接口层原有那道不动，三层文案逐字一致）；负数不是 0 的重复，它能把别的明细**对冲**掉。单测 401、脚本 393（orders +C2b），见条目内「拍板与第三道闸」 |
...
| 48 | 前端 | 缺一半（**没有任何自动检查会管这件事**） | 默认账号 `admin`（管理员）登进去**处处 403**：ADMIN 唯一能干的 `/api/staff` 六个 + 门店写四个，**前端一个页面都没有**，而登录页偏偏把他设成默认值。build 不报（页面不存在就没有东西会编译错）、单测不报（测的是后端）、九脚本不报（接口全是好的）—— 它们都只检查"已有的对不对"，**没有一样在问"该有的在不在"**。修法：补 `/staff/admin/*` 两页 + `router.homeFor` 按 role 分流落地页 + 默认账号改 `manager` + `meta.adminOnly` 只拦店长那边。**本轮只做到构建通过 + 逐字核对源码，页面一次没跑过**。**2026-09-19 当晚补记**：真机点过了 —— 无头 Chrome 44 条断言全过（探针装在仓库外），CDN 偶发失败同批挂上备用链；**紧接着发现下一条 49：页面有了，里面还有一根线没接** |
| 49 | 前端 | **潜伏缺陷**（并发下悄悄改错别人的数据） | 编辑面板拿**名册行**回显，保存却交**整份快照** → 管理员 A 开着页面时 B 改了员工 X 的手机号，A 只改姓名一保存，**把 B 的改动悄悄改回去了**（不报错、不留痕；单人单设备用不出来）。防这件事的 `GET /api/staff/{id}`：后端活着、包装函数写好、注释就写着"编辑面板回显用" —— **就是没人 import**。是**数引用**数出来的，不是点出来的（`npm run build` 对"导出了没人用"一言不发）。修法：`openEdit` 拆两段 —— 先用行秒开面板、再 `loadFresh()` 取权威快照覆盖，带竞态守卫与失败降级；21 条断言真机过（含拦包证明"表单来自响应"、详情字段集与名册行一致这个 `storeId` 危险面）。**并发那一幕本身没导演过** |
