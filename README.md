...
实现过程中的 **30 条踩坑记录**在 [`docs/bug-record.md`](docs/bug-record.md) —— 那份文档比这份 README 更能说明这个项目做了什么。
...
| `admin` | `admin123` | **管理员**。按设计只负责"人与店"的管理，但**员工 / 门店管理接口尚未实现**，目前除读价目表、门店列表外一律 403（订单与定价写接口的 403 是 2026-09-11 的有意口径） |
...
八个脚本分别证明什么、断言数是多少、跑之前要注意哪些坑，见 [`scripts/README.md`](scripts/README.md)。**脚本会往库里写数据**，要干净数据只能 `down -v` 重建（哪些脚手架必须留着，那份 README 里有清单）。
...
- [`docs/bug-record.md`](docs/bug-record.md) —— 30 条 bug 的现象 / 根因 / 修法，以及每轮"测试通过的功能"清单
