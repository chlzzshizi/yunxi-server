...
| 48 | 前端 | 缺一半（**没有任何自动检查会管这件事**） | 默认账号 `admin`（管理员）登进去**处处 403**：ADMIN 唯一能干的 `/api/staff` 六个 + 门店写四个，**前端一个页面都没有**，而登录页偏偏把他设成默认值。build 不报（页面不存在就没有东西会编译错）、单测不报（测的是后端）、九脚本不报（接口全是好的）—— 它们都只检查"已有的对不对"，**没有一样在问"该有的在不在"**。修法：补 `/staff/admin/*` 两页 + `router.homeFor` 按 role 分流落地页 + 默认账号改 `manager` + `meta.adminOnly` 只拦店长那边。**本轮只做到构建通过 + 逐字核对源码，页面一次没跑过** |
